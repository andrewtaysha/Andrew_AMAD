...
class ListsTableViewController: UITableViewController {
    ...
    @IBAction func doneNewListSegue (_ segue:UIStoryboardSegue){
        if segue.identifier=="doneSegue"{
            let source = segue.source as! AddListViewController //only add a country if there is text in the textfield
            if ((source.addedList.isEmpty) == false){
            countries.append(source.addedList)
            tableView.reloadData()
            let chosenContinent = continentListDetail.continents[selectedContinent]
            continentListDetail.continentData[chosenContinent]?.append(source.addedList)
        }
    }
}*/
