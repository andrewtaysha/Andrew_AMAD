...
class DetailListTableViewController: UITableViewController {
    ...
    @IBAction func unwindEditListItemSegue (_ segue:UIStoryboardSegue){
        if segue.identifier=="SaveEditListItemSegue"{
            let source = segue.source as! EditListItemViewController //only add a list item if there is text in the textfield
            if ((source.editedListItem?.isEmpty) == false){
                listItems.append(source.editedListItem!)
                tableView.reloadData()
                let chosenListName = listNameListDetail.listName[selectedListName]
                listNameListDetail.listNameData[chosenListName]?.append(source.editedListItem!)
            }
        }
    }
    ...
    @IBAction func saveToDetailListTableViewController (segue:UIStoryboardSegue) {
        let detailViewController = segue.source as! EditListItemViewController
        
        let index = detailViewController.index
        
        let editedString = detailViewController.editedListItem
        
        listItems[index!] = editedString!
    ...
    func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
     ...
     let detailViewController = segue.destination as! EditListItemViewController
